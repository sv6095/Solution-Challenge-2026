...
# ---------------------------------------------------------------------------
# Sea Route Endpoint — computes realistic maritime route via shipping lane
# waypoints, avoiding land masses, returning GeoJSON coordinates.
# ---------------------------------------------------------------------------
...
@app.post("/api/sea-route")
async def api_sea_route(payload: SeaRouteRequest) -> dict:
    """
    Compute a realistic maritime sea route between two coordinates.
    Uses the searoute library (Eurostat) if available, otherwise falls back to
    a curated maritime waypoint graph covering major shipping lanes and chokepoints.
    ...
    # Try searoute library first (installed via pip install searoute==1.6.0)
    # Pylance may show a false "module not found" if its cache is stale — runtime is fine.
    # Fix: VS Code → Ctrl+Shift+P → "Python: Restart Language Server"
    try:
        ...
        origin = [from_lng, from_lat]   # searoute uses [lng, lat]
        destination = [to_lng, to_lat]
        route = sr.searoute(origin, destination, units="km")
        coords = route["geometry"]["coordinates"]
        # coords are [lng, lat] pairs — convert to [[lng, lat]] for MapRoute
    ...
    # Fallback: curated maritime chokepoint waypoint graph
    # Major maritime waypoints / chokepoints used in global shipping
    WAYPOINTS = {
        # Atlantic
    ...
    def _interpolate_sea(lat1: float, lon1: float, lat2: float, lon2: float, steps: int = 20) -> list:
        """Interpolate along a straight sea segment."""
        return [
    ...
    def _choose_waypoints(flat: float, flng: float, tlat: float, tlng: float) -> list:
        """
        Heuristic: pick minimal set of chokepoints so the route stays at sea.
        Basin detection uses longitude / latitude bands.
        """
        wps = []

        # Europe / Mediterranean → Indian Ocean / Asia: go through Suez
        ...
        # Southern hemisphere / Cape of Good Hope
        if (is_southern or is_southern_dest) and not (
            ...
            if (is_americas or is_americas_dest) and (is_africa := (30 < flng < 55 or 30 < tlng < 55)):
                wps += ["cape_good", "s_atlantic"]
            elif is_pacific or is_pacific_dest:
                wps += ["cape_horn"]

        # Taiwan Strait for NE Asia routes
        if (flng > 120 and flat > 20) and (tlng > 120 and tlat > 20):
            wps += ["taiwan_str"]

        return wps

    # Build the coordinate path via waypoints
    ...
    # Interpolate between each consecutive pair for a smooth curve
    coords: list = []
    total_km = 0.0
...
# ---------------------------------------------------------------------------
# Land Route Endpoint — Real road routing
# Priority: 1) Valhalla truck routing (OSM)  2) OSRM car routing  3) Estimate
...
def _decode_polyline6(encoded: str) -> list[list[float]]:
    ...
    while index < len(encoded):
        ...
        result.append([round(lng / 1e6, 6), round(lat / 1e6, 6)])  # [lon, lat]
    return result


@app.post("/api/land-route")
async def api_land_route(payload: LandRouteRequest) -> dict:
    """
    Compute a real land/road freight route with cascading real-API fallback:
      1. Valhalla truck routing — valhalla1.openstreetmap.de (OSM, free, no key)
         Uses the truck costing profile for HGV-appropriate road network.
      2. OSRM car routing — router.project-osrm.org (free, no key)
      3. Straight-line estimate with 1.35× road factor (last resort).
    Routes > 6,000 km straight-line are rejected as transoceanic.
    """
    import math, httpx
    ...
    # ── 1. Valhalla truck routing (OSM public instance, free, no key) ─────────
    try:
        ...
        valhalla_body = {
            ...
            "costing_options": {
                "truck": {
                    "weight":    21.77,   # tonnes
                    "axle_load": 9.07,   # tonnes per axle
                    "height":    4.11,   # metres
                    "width":     2.60,
                    "length":   21.64,
                }
    ...
    # ── 2. OSRM car routing (public instance, free, no key) ──────────────────
    try:
        osrm_url = (
    ...
    # ── 3. Straight-line estimate (last resort) ───────────────────────────────
    steps = 60
    coords = [
        [
    ...
    road_km = round(straight_km * 1.35, 1)  # road ≈ 1.35× crow-flies
    return {
        "viable": True,
        "coordinates": coords,
        "distance_km": road_km,
        "duration_hours": round(road_km / 80, 1),  # 80 km/h avg HGV speed
        "source": "estimate",
...
# ---------------------------------------------------------------------------
# Air Route Endpoint — OurAirports real database + great-circle paths
# Airport data: https://ourairports.com/data/airports.csv (public domain)
# Cached in memory at startup. Falls back to curated cargo hub list.
# ---------------------------------------------------------------------------

# Module-level cargo hub cache (populated lazily on first air-route request)
_AIR_HUB_CACHE: list[dict] | None = None
_AIR_HUB_FETCH_LOCK: "asyncio.Lock | None" = None

# Curated top-50 global cargo hubs (IATA, name, city, lat, lng)
# Source: IATA World Air Transport Statistics 2024, Boeing World Air Cargo Forecast
_CARGO_HUBS_BUILTIN: list[dict] = [
    ...
    {"iata": "BOG", "name": "Bogotá El Dorado",          "city": "Bogotá",       "country": "CO", "lat": 4.702,  "lng": -74.147},
...
async def _get_cargo_hubs() -> list[dict]:
    """
    Returns cargo hub airport list. Tries to fetch + cache a filtered set from
    OurAirports (public domain CSV) on first call; falls back to builtin list.
    OurAirports CSV: https://ourairports.com/data/airports.csv
    """
    ...
    async with _AIR_HUB_FETCH_LOCK:
        if _AIR_HUB_CACHE is not None:   # double-checked
            return _AIR_HUB_CACHE
        try:
            ...
            hubs: list[dict] = []
            # Only keep large airports with an IATA code; prefer major cargo hubs
            for row in reader:
                ...
                try:
                    ...
                    continue
                # Keep only if it's in our builtin cargo hub list OR a large airport
...
@app.post("/api/air-route")
async def api_air_route(payload: AirRouteRequest) -> dict:
    """
    Returns great-circle arc coordinates and real cargo hub airports for an air
    freight route. Airport data sourced from OurAirports (public domain CSV,
    ~15k airports) cached in memory. Falls back to curated builtin hub list.

    Note: great-circle paths ARE the real algorithm for air routes — aircraft
    follow great-circle (orthodromic) paths to minimise distance and fuel.
    """
    import math
    ...
    def _arc(lat1: float, lon1: float, lat2: float, lon2: float, steps: int = 80) -> list:
        """Great-circle arc (orthodrome) — actual flight path algorithm."""
    ...
    # Find a plausible mid-point transit hub (avoids duplicating origin/dest hubs)
    alt_hub = None
...
# ---------------------------------------------------------------------------
# Route Cost Endpoint — realistic freight cost estimates for all modes
# ---------------------------------------------------------------------------
...
async def api_route_cost(payload: RouteCostRequest) -> dict:
    """
    Returns approximate real-world freight cost in USD for a given mode and distance.

    Cost models (industry averages, 2024):
      Air:   $3.50–6.00/kg depending on distance tier + fuel surcharge
      Sea:   $800 base (port fees) + $0.18–0.25/km per TEU equivalent (24t)
      Land:  $200 base + $1.80–2.50/km per truck (20t payload)
      Rail:  $150 base + $0.90–1.20/km per wagon (60t)
      Hybrid (sea + land): sea leg ~70% of distance + land transfer
    """
    ...
    if mode == "air":
        # Air cargo rates: distance-tiered, $/kg
        if km < 500:
            rate_per_kg = 5.80   # domestic / short haul
        elif km < 3000:
            rate_per_kg = 4.20   # regional
        elif km < 8000:
        ...
        # Sea freight per TEU (24,000 kg). Scale for actual weight.
        ...
        port_fees = 800 * teu   # origin + destination port
        fuel      = 0.06 * km * teu  # bunker surcharge
        ...
        # Road freight per truck (20t payload). Scale for weight.
        ...
        # Sea for ~70% of the distance + land for remaining 30%
        sea_km  = km * 0.70
        land_km = km * 0.30
        teu     = max(1.0, kg / 24000.0)
        trucks  = max(1.0, kg / 20000.0)
